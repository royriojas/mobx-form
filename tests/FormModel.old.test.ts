...
describe("form-model", () => {
  describe("restoreInitialValues", () => {
    ...
    it("should reset the initial values on the form even if not empty strings", () => {
      const model = createModelFromState<{ valueA: number[]; valueB: number[] }>({
        valueA: [],
        valueB: [],
      });
  ...
  describe("requiredAreFilled", () => {
    it("should be true if all required fields have a value", () => {
      ...
      const model = createModelFromState(initialState, {
      ...
      const stop = reaction(
        ...
        ({ fields }) => {
          ...
          fields.email.setRequired(
            fields.lastName.value === "Doo" && "Email is required for all Doos",
          );
        },
      );
      ...
      expect(model.requiredFields.sort()).toEqual(
        ["name", "lastName"].sort() as (keyof typeof initialState)[],
      );

      model.updateField("lastName", "Doo");

      expect(model.requiredFields.sort()).toEqual(
        ["name", "lastName", "email"].sort() as (keyof typeof initialState)[],
      );
      expect(model.requiredAreFilled).toBe(false);
    ...
    it("should allow validators to access fields in the model", async () => {
      type CustomModel = FormModel<{ name: string; lastName: string; email: string }> & {
  ...
  describe("addFields", () => {
    ...
    it("should store non recognized fields as meta in the fields", async () => {
      ...
      expect(fields.numOfBedrooms.errorMessage).toEqual(
        descriptors.numOfBedrooms.required as string,
      );
